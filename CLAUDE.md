...
- 装飾（タイムコード等）はスマホで `hidden sm:block`。
- **日本語は語・文節の途中で折らない（2026-09-26 本人ルール）**。body の `word-break: auto-phrase` は **iPhone の Safari が非対応**で、「植／物」「グ／リーン」「か／ら」のように切れる。
  - 本文・説明・仕様の値・メモ（長い文）は `<Jp>`（`src/components/jp.tsx`、BudouX をサーバーで適用）で包む。
  - 物件名など短い見出しは `<TitleText>`（`src/components/title-text.tsx`、Intl.Segmenter で語ごとに inline-block。client component でも使える）。
  - スマホで「ラベル｜値」の2列表を作らない。640px 未満はラベルの下に値を全幅で置く（値の列が半分だと数文字ごとに折れる）。
  - `design-fb-audit.py` が物件ページ・カード・データ販売で守られているか検査する。ナビは `lg` 未満ハンバーガー（`MobileNav`）。ブランド中央寄せは `flex-1` ゾーンで（absolute 中央は重なるので不可）。

## 物件「概要」の制作ルール（2026-09-19）

概要＝**フレーバーテキスト**（空気感・見どころ・撮れる画。根拠は公式サイトの紹介文）。
駅からの分数・広さ・天井高・営業時間・料金・駐車場など**仕様欄と重複する情報は書かない**。
3〜6文・1文40字前後。表示は句点ごとに自動改行（`sentenceLines`）。
詳細と書き換え例: `docs/property-overview-copy-rules-2026-09-19.md`。
