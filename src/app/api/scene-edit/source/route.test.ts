...
it('rejects unauthenticated, other actor, revoked access and changed raw snapshot before storage',async()=>{
 ...
 mocks.access.mockResolvedValue({id:'owner'});sqlite.exec("UPDATE properties SET data=data||' '");expect((await GET(new Request(url))).status).toBe(409);expect(mocks.get).not.toHaveBeenCalled();
});
