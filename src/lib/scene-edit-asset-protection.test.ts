...
const state=vi.hoisted(()=>({binding:JSON.stringify({purpose:'scene-edit-v1',target:{expiresAt:new Date(Date.now()+3600000).toISOString()}})}));
...
it('releases an expired, unattached reservation after the grace period',async()=>{
 state.binding=JSON.stringify({purpose:'scene-edit-v1',target:{expiresAt:new Date(Date.now()-3*24*3600000).toISOString()}});
 expect(await sceneEditAssetProtection({id:'wf_'+'b'.repeat(64),url:'/trimmed.zip'},async()=>[])).toBeNull();
});
